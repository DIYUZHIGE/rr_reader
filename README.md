...
- 基础 Reader：打开并显示一个 Markdown 文件的开头
...
### 当前 Reader 不是完整 Markdown 渲染器

现在 Reader 只是把 Markdown 文件内容按字体宽度绘制到屏幕，超过一屏的内容会被截断。下一步应实现分页，再开始处理 Markdown 语法。
...
2. 实现分页：把一个 Markdown 文件按屏幕大小切成多页。
3. Reader 翻页：侧键在单个文件内翻页，而不是切换文件。
4. 保存阅读进度：记录每个文件最后阅读页。
5. 文件浏览器目录化：支持文件夹展开/进入，而不是扁平路径列表。
6. Markdown 基础渲染：标题、列表、引用、代码块、链接文本。
7. Obsidian 特性：`[[wiki link]]`、标签、callout。
8. WiFi + S3 同步：读取 remotely-save 配置并下载 vault。

...
const READER_HALF_REFRESH_INTERVAL: i32 = 15;
...
pub struct ReaderApp {
    ...
    reader_refreshes_until_half: i32,
}
...
impl ReaderApp {
    ...
    fn open_selected_file(&mut self) {
        ...
        self.render_current_file();
        self.reset_reader_refresh_cycle();
        self.flush_reader_refresh_cycle();
    }
    ...
    fn reset_reader_refresh_cycle(&mut self) {
        self.reader_refreshes_until_half = 0;
    }

    fn flush_reader_refresh_cycle(&mut self) {
        let mode = if self.reader_refreshes_until_half <= 1 {
            self.reader_refreshes_until_half = READER_HALF_REFRESH_INTERVAL;
            RefreshMode::Half
        } else {
            self.reader_refreshes_until_half -= 1;
            RefreshMode::Fast
        };

        if let Err(e) = self.display.flush_with_mode(mode) {
            warn!("Reader refresh failed: {}", e);
        }
    }

...
impl ReaderApp {
    ...
    fn handle_input(&mut self) -> Result<()> {
        ...
        match self.activity.clone() {
            Activity::FileBrowser(_) => {
                if self.hardware.input.any_logical_pressed(&[Down, PageForward]) {
                    self.move_browser_selection(1);
                }

                if self.hardware.input.any_logical_pressed(&[Up, PageBack]) {
                    self.move_browser_selection(-1);
                }
    ...
    fn open_adjacent_file(&mut self, delta: isize) {
        ...
        let next = if delta.is_negative() {
            ...
            current.saturating_add(delta as usize).min(self.md_files.len() - 1)
        };
    ...
    fn render_file_browser(&mut self) {
        ...
        let summary = format!("{} files", self.md_files.len());
        self.display
            .draw_text_font(&self.font, &summary, 20, 32);

        for (row, idx) in (first_visible..end).enumerate() {
            let marker = if idx == selected { ">" } else { " " };
            let path = Self::truncate_for_width(
                &self.md_files[idx],
                Display::width() - LIST_X - self.font.glyph_width as usize,

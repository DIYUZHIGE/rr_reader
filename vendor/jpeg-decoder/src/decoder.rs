...
fn compute_image(
    ...
    if components.len() == 1 {
        crop_component_plane(&components[0], data.remove(0), output_size)
    } else if color_transform == ColorTransform::Grayscale {
        crop_component_plane(&components[0], data.remove(0), output_size)
    } else {
...
fn crop_component_plane(
    ...
    // If the image width is a multiple of the block size, the rows are already packed.
    if usize::from(output_size.width) != line_stride {
        // Move from top to bottom because the destination is before the source.
        for y in 1..height {
